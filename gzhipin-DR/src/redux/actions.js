...
// 注册/登陆成功的同步action
//在这个函数的类型为AUTH_SUCCESS,数据携带在action上传输给reducers(action.data)
...
export function register({username, password, password2, type}) {
  ...
  return async dispatch => {
    // 执行异步(发送ajax请求)
    //reqRegister调用这个函数在api中,用于发送ajax请求
    const response = await reqRegister({username, password, type})
    // 异步得到结果,
    const result = response.data // {code: 0, data: user} | {code: 1, msg: 'xxx'}这是后台定义好的
    //判断得到的code是否等于0,若等于0.则表示数据请求成功,data的值user中包含用户的数据
    if (result.code === 0) { // 成功
      // 分发同步action(成功)
      ...
      debugger
      //调用同步action中的函数authSuccess,将user传输过去(authSuccess在本文件的上方)
...
创建一个函数,分别在注册成功,登录成功,异步获取用户信息时调用
这个函数的主要功能是传输聊天中的数据
